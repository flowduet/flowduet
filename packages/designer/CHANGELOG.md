...
- fd20221: BPMN 只读画布默认以可读缩放展示流程顶部，并把事件和网关名称移出纵向连接线。
- 9b0a48e: 条件分支在钉钉式视图显示分支名称、条件表达式和默认兜底，在 BPMN 只读视图提供条件短标签与完整表达式查看入口。
- 6ca22fc: 钉钉式分支块补引入引出线（#51）：每条支路列顶部引入竖线（末端向下箭头指向卡片，对照钉钉官方分叉语义）、底部引出竖线收拢——条件块与并行块统一生效。
- a969f8c: 导出前统一校验审批、抄送和条件分支草稿：未配置字段一次列出全部可定位问题，修正后再生成 XML。
- e6289c8: 钉钉式节点卡片名称与摘要在窄支路截断时仍可通过悬停查看完整文本。
